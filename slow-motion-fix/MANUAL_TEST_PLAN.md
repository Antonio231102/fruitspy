...
An update installation works when the installed APK and candidate use the same package name and persistent local FruitSpy signing key:
...
If the device has the same package signed by the original publisher or another key, preserve its data before uninstalling it. Android rejects an update whose signing identity differs. A different custom package can be installed alongside that application without uninstalling it, but does not inherit its private saves.
