...
- **The preferred-name restoration revision is installed and hash-verified on the S20 and S4; physical-pair live qualification is pending.** Both phones execute `armeabi-v7a`. The earlier callback-only APK completed five Wi-Fi/cellular matches with the x86 emulator and S20 hosting in turn, but those results do not qualify the revised connection hook.
