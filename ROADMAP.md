...
- [x] Add independent optional package-name and launcher-display-name choices, with separate empty-to-skip prompts after the server address and before native patch progress. Package customization synchronizes the compiled manifest and resource-table namespace while preserving resource IDs, classes and JNI bindings. All four option combinations passed signed-build, Android parser and entry-difference checks; the guided CLI and UTF-16/package-capacity boundaries passed. A subsequent custom-package build was installed on the emulator alongside the original package, and the user confirmed successful launch and expected initial operation; multiplayer and extended-session behavior were not established by that report.
- [x] Name default output APKs `<launcher name> FruitSpy - <IP/hostname>.apk`, without the duplicated prefix when the launcher name is exactly `FruitSpy`. Skipped launcher customization uses `Fruit Ninja`; explicit output paths retain precedence. Sixteen patcher tests passed, and actual CLI builds verified the default, custom, exact-`FruitSpy`, and explicit-path naming cases.
...
- [x] Name the command-line tool **FruitSpy Patcher** in its startup banner, help, error/completion messages and diagnostic report. Keep the script at `patcher/patch_apk.py` and preserve APK naming and signing identity.
