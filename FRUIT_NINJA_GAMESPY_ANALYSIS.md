...
Reproduce the complete regression suite from the repository root with `python -m unittest discover -s patcher/tests -v` and the privately supplied allowlisted APK in its usual location; real-APK cases skip without that input. The native JNI case additionally uses the optional Unicorn dependency pinned in `nickname-collision-fix/requirements-validation.txt`; install it with `python -m pip install -r nickname-collision-fix/requirements-validation.txt` when native execution is needed. Without Unicorn, that execution case skips; ordinary patching remains standard-library-only. The latest recorded run passed all 22 patcher tests, including native execution with `PYTHONPATH=nickname-collision-fix/build/deps`.

The default workflow aligns and signs the generated APK with a random persistent per-user key stored as `signing.p12` and password-bearing `signing.json` (Windows default: `%LOCALAPPDATA%\FruitSpy`). It enables v1/v2/v3 signing with v4 disabled, verifies alignment and signatures, and requires legacy-compatible JAR-v1. No shared project signing identity is distributed. The recent actual interactive signed build verified FruitSpy Patcher branding, conditional LVL removal, reuse of that existing identity, and v1/v2/v3 signatures. An explicit `--unsigned` mode retains external-signing support. Runtime tracing of earlier equivalent endpoint builds confirmed NatNeg exchange with the configured server and direct peer traversal; the hardened clock build completed direct and relayed games through the ARMv7 runtime path. Existing runtime qualification is recorded in [slow-motion-fix](slow-motion-fix/README.md) and [nickname-collision-fix](nickname-collision-fix/README.md); it is not a runtime test of the optional identity changes and does not imply a new compatibility-testing campaign or publication approval.
